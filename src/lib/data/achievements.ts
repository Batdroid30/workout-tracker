...
/**
 * Computes earned/unearned achievement badges from the user's training history.
 * All data is derived from existing tables — no new DB columns needed.
 */
export async function getBadges(
  ...
  // Compute longest-ever streak (weeks)
  ...
  for (const week of weeks) {
    if (prevWeek === null) {
      ...
      const nextExpected = new Date(prevWeek)
      nextExpected.setUTCDate(nextExpected.getUTCDate() + 7)
      runningStreak = week === nextExpected.toISOString().split('T')[0] ? runningStreak + 1 : 1
    }
    if (runningStreak > longestStreak) longestStreak = runningStreak
    prevWeek = week
  }

  // Build badge list — order: workout → volume → PR → streak
  const badges: Badge[] = [
